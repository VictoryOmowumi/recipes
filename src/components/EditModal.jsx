...
const EditModal = ({showEditModal, setShowEditModal, name, description, image, setName, setDescription, setImage, editRecipe, id}) => {
  return (
    <div className='fixed top-0 left-0 w-full h-full bg-black bg-opacity-50 z-50 transition-opacity duration-300 flex justify-center items-center'>
            ...
            <div className='w-full flex justify-end'>
                <div className='bg-teal-400 p-3 rounded-full hover:border hover:bg-white hover:border-teal-400 transition-colors ease-in-out '>
                    <IoMdClose className='text-2xl cursor-pointer text-white hover:text-teal-400 transition-colors ease-in-out' onClick={() => setShowEditModal(false)}/>
                </div>
            </div>
